function output = dT_dt_k_solver_ver3(T,T_inlet,p_profile,D_hydraulic,r_inner,x_step,volume_heater,density_steel, A_HS,vol_fluid, inner_assembly_mass,mass_flow_fluid,n_segments,A_insulation, U_insulation,T_air,k) 

T_heater = T(:,1);
T_fluid = T(:,2);

%Guess k_heat_coefficient and steel coefficient 
k_heat_coefficient = k;
...
dT_dt_heater = (1./(density_steel.*volume_heater.*c_p_steel(T_heater))).*((-volume_heater.*k_steel(T_heater).*d2T_dx2) + (p_profile-total_heater_power_loss) - (k_heat_coefficient.*A_HS).*(T_heater - T_fluid));
...
dT_dt_fluid = (1./heat_cap_fluid).*((mass_flow_fluid.*c_p_oil(T_fluid).*(T_fluid_calc-T_fluid)) + (k_heat_coefficient.*A_HS).*(T_heater - T_fluid));
